...
export default function Prediction() {
  ...
  const saveToDatabase = async (predictionResult) => {
    try {
      const dataToSave = {
        ...form,
        prediction: predictionResult.classification,
        diagnosis: `Risk: ${predictionResult.classification} (${predictionResult.probability}%)`
      };
      ...
      if (data.success) {
        setSaveStatus({
          type: "success",
          message: `✓ Data saved successfully! Patient ID: ${data.patient_id}`,
        });
      ...
      setSaveStatus({
        type: "error",
        message: "Failed to connect to server. Data not saved.",
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save to database and trigger analytics regeneration
      ...
      <div className="w-screen px-4 py-10 bg-gradient-to-b from-purple-50 via-pink-50 to-teal-50 min-h-screen">
        <section className="max-w-4xl mx-auto mt-10 bg-white/80 backdrop-blur-md border border-pink-200 p-8 rounded-2xl shadow-xl">
          ...
          <form
            ...
            className="bg-white shadow p-6 rounded-2xl w-full max-w-2xl mx-auto"
          >
            {/* Numeric Inputs */}
          ...
          {result && (
            <div className="mt-8 bg-white p-6 rounded-2xl shadow-lg max-w-lg text-center mx-auto">
              ...
              <p className="text-sm text-gray-500 mt-4">
                ⚠️ This is for educational purposes only. Consult a healthcare
        ...
        <section className="max-w-4xl mx-auto mt-10 bg-gradient-to-r from-teal-50 to-pink-50 border border-purple-100 p-6 rounded-2xl shadow-sm mb-20 text-center">
          <p className="text-lg font-medium text-gray-700">
            Output — maybe we refer them to a website to find doctors in their
            area.
          </p>
        </section>
      </div>
